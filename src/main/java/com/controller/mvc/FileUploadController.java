...
public class FileUploadController {
    ...
    public FileDesc uploadFileHandler(@RequestParam("file") MultipartFile file, @RequestParam("path") String path, HttpServletResponse response) throws IOException {
        ...
        if (!file.isEmpty()) {
            ...
            Path folderPath = Paths.get(ConstantPara.UPLOAD_FOLDER + File.separator + path);

            if(Files.notExists(folderPath))
                Files.createDirectories(folderPath);


            Path filePath = Paths.get(ConstantPara.UPLOAD_FOLDER + File.separator + path + File.separator + file.getOriginalFilename());
